...
# define the labels (COCO data set)
...
color_labels = {i: list(np.random.random(size=3) * 256) for i in labels}
...
def decode_netout(netout, anchors, obj_thresh, net_h=416, net_w=416):
...
def draw_boxes(filename, v_boxes, v_labels, v_scores):
    ...
    for i in range(len(v_boxes)):
        ...
        thickness = (data.shape[0] + data.shape[1]) // 600
        data = cv2.rectangle(data, (x1, y1), (x1 + width, y1 + height), color_labels[v_labels[i]], thickness)
        label = "%s (%.2f)" % (v_labels[i], v_scores[i])
        # get text size
        (test_width, text_height), baseline = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, thickness // 3, 1)
        # put text rectangle
        cv2.rectangle(data, (x1, y1), (x1 + test_width, y1 - text_height - baseline), color_labels[v_labels[i]],
                      thickness=cv2.FILLED)
        # put text above rectangle
        cv2.putText(data, label, (x1, y1 - 2), cv2.FONT_HERSHEY_SIMPLEX, thickness // 3, (0, 0, 0), 3)
        print(data,label)
    # plot the image
    pyplot.imshow(data)
    # show the plot
...
def yolo_decode_boxes(yhat, class_threshold, image_h, image_w, input_h=416, input_w=416):
    boxes = list()
